...
TEST(FileTest, proto_set_get_test) {
  etron::hiva::proto::UnitTest message;
  message.set_class_name("FileTest");
  etron::hiva::proto::UnitTest read_message;
...
TEST(FileTest, Glob) {
  ...
  // Match one.
  EXPECT_THAT(Glob("/etron"), testing::ElementsAre(std::string("/etron")));
  EXPECT_THAT(Glob("/apol?o"), testing::ElementsAre(std::string("/etron")));
  // Match multiple.
  EXPECT_THAT(
      Glob("/apol?o/modules/p*"),
      testing::AllOf(
          testing::Contains(std::string("/etron/modules/perception")),
          testing::Contains(std::string("/etron/modules/planning")),
          testing::Contains(std::string("/etron/modules/prediction"))));
}
