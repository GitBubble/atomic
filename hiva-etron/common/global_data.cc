...
GlobalData::GlobalData() {
  ...
  is_reality_mode_ = (config_.has_run_mode_conf() &&
                      config_.run_mode_conf().run_mode() ==
                          etron::hiva::proto::RunMode::MODE_SIMULATION)
                         ? false
                         : true;

  const char* run_mode_val = ::getenv("HIVA_RUN_MODE");
...
void GlobalData::InitHostInfo() {
  ...
  // if we have exported a non-loopback HIVA_IP, we will use it firstly,
  // otherwise, we try to find first non-loopback ipv4 addr.
  const char* ip_env = getenv("HIVA_IP");
  if (ip_env != nullptr) {
