...
TEST(MessageTraitsTest, message_type) {
  ...
  EXPECT_EQ(msg_type, "etron.hiva.proto.UnitTest");
...
TEST(MessageTraitsTest, descriptor) {
  const std::string pb_desc =
      ...
      "\x1(\x4\x12\xF\n\acontent\x18\x3 \x1(\t";
  std::string desc;
  GetDescriptorString<proto::UnitTest>("etron.hiva.proto.UnitTest", &desc);
  EXPECT_EQ(pb_desc, desc);

  desc = "";
  GetDescriptorString<RawMessage>("etron.hiva.proto.UnitTest", &desc);
  EXPECT_EQ(pb_desc, desc);

  desc = "";
  GetDescriptorString<Data>("etron", &desc);
  EXPECT_EQ("", desc);

  desc = "";
  GetDescriptorString<Message>("etron", &desc);
